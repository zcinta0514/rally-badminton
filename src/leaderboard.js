...
export function createLeaderboard({ document, getURL, fetchImpl = globalThis.fetch }) {
  ...
  async function load({ selfId } = {}) {
    ...
    try {
      ...
      $('leaderboard-status').textContent = data.entries.length ? `前 ${Math.min(50, data.entries.length)} 位球友 · 完成好友对局后更新` : '暂无战绩。完成一场好友 1V1，即可上榜。';
      $('leaderboard-storage').textContent = data.storage === 'memory' ? '当前是临时排行榜，服务器重启后可能丢失。' : '统计本服务器已完成的好友对局；人机练习和中途退出不计入。';
    } catch {
      if (current !== generation) return;
      $('leaderboard-rows').replaceChildren(); $('leaderboard-table').hidden = true;
