export function getWebSocketURL(page = globalThis.location, config = globalThis.RALLY_CONFIG || {}) {
  if (config.demoMode === true) throw new Error('本页可人机练习；好友对打请先从首页进入局域网游戏。');
...
export function initPWA({ fullscreenButton, showToast = () => {} } = {}) {
  ...
  function updateUI() {
    ...
    if (!globalThis.isSecureContext) {
      ...
      long = `本机离线资源已备好${version ? '（' + version.slice(0,8) + '）' : ''}。断网后仍可从同一网址或主屏幕进入人机。${demoMode ? '好友对打请先从首页进入局域网游戏。' : '好友对打需要与运行游戏的电脑保持网络连接。'}浏览器清理存储后需重新联网下载。`;
    } else if (failure) {
